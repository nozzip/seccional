...
export default function PoolSchoolGrid() {
    ...
    const [view, setView] = useState(0); // 0: Grid, 1: Summary
    const [selectedSlot, setSelectedSlot] = useState<{ day: string; hour: string } | null>(null);

    const studentsInSelectedSlot = selectedSlot ? mockPoolData[`${selectedSlot.day}-${selectedSlot.hour}`] || [] : [];
        ...
        <Box>
            <Box sx={{ mb: 4, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { sm: 'center' }, gap: 2 }}>
                <Box>
                    ...
                    <Tabs value={view} onChange={(_, v) => setView(v)} sx={{ minHeight: 40, mt: 1 }}>
                        <Tab icon={<ViewModuleIcon sx={{ fontSize: 20 }} />} label="Grilla Horaria" iconPosition="start" sx={{ minHeight: 40 }} />
                        <Tab icon={<DescriptionIcon sx={{ fontSize: 20 }} />} label="Resumen Profesores" iconPosition="start" sx={{ minHeight: 40 }} />
                    </Tabs>
                </Box>
                <Button variant="contained" startIcon={<AddIcon />} sx={{ alignSelf: 'flex-end' }}>
                    Inscribir Alumno
                </Button>
            </Box>
                ...
                <Box sx={{ overflowX: 'auto' }}>
                    <Box sx={{ minWidth: 1000 }}>
                        ...
                        <Grid container spacing={1} sx={{ mb: 1 }}>
                            <Grid item xs={1}><Box sx={{ height: 40 }} /></Grid>
                            {DAYS.map(day => (
                                <Grid item xs={1.8} key={day}>
                                    <Paper elevation={0} sx={{ p: 1, textAlign: 'center', bgcolor: alpha(theme.palette.primary.main, 0.1), borderRadius: 2 }}>
                                        ...
                                        <Grid item xs={1.8} key={day}>
                                            <Paper
                                                ...
                                                sx={{
                                                    ...
                                                    border: '1px solid',
                                                    borderColor: students.length > 0 ? 'secondary.main' : 'divider',
                                                    borderRadius: 2,
                                                    bgcolor: students.length > 0 ? alpha(theme.palette.secondary.main, 0.05) : 'transparent',
                                                    cursor: 'pointer',
                                                    transition: 'all 0.2s',
                                                    '&:hover': {
                                            ...
                                            >
                                                {students.length > 0 ? (
                                                    <Box sx={{ textAlign: 'center' }}>
                                                        <Typography variant="h6" sx={{ color: 'secondary.dark', fontWeight: 800, lineHeight: 1 }}>
                                                            {students.length}
                                                        </Typography>
                        ...
                        <Grid item xs={12} md={6} lg={4} key={prof}>
                            <Card sx={{ borderRadius: 3, border: '1px solid', borderColor: 'divider' }} elevation={0}>
                                <CardHeader
                                    ...
                                    title={<Typography variant="h6" sx={{ fontWeight: 800 }}>{prof}</Typography>}
                                ...
                                <List sx={{ p: 0 }}>
                                    {students.map((s, idx) => (
                                        <ListItem key={s.id} divider={idx !== students.length - 1}>
                                            <ListItemText
                                                primary={<Typography sx={{ fontWeight: 700 }}>{s.name} {s.lastName}</Typography>}
            ...
            >
                        ...
                        <List sx={{ pt: 0 }}>
                            {studentsInSelectedSlot.map(s => (
                                <ListItem key={s.id} sx={{ px: 0 }}>
                                    <ListItemAvatar>
                                        <Avatar sx={{ bgcolor: 'secondary.main' }}>{s.name.charAt(0)}</Avatar>
                                    </ListItemAvatar>
                                    <ListItemText
                                        primary={`${s.name} ${s.lastName}`}
                                        secondary={<Box component="span" sx={{ color: 'primary.main', fontWeight: 700 }}>{s.professor}</Box>}
                                    />
                                </ListItem>
                            ))}
                        </List>
                ...
                <DialogActions sx={{ p: 2 }}>
                    <Button onClick={() => setSelectedSlot(null)} fullWidth variant="outlined">Cerrar</Button>
                    <Button color="primary" variant="contained" fullWidth startIcon={<AddIcon />}>Inscribir</Button>
                </DialogActions>
            </Dialog>
        </Box>
    );
