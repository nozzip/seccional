  ...
  activeStudents.forEach((student: StudentData) => {
    Object.keys(student.schedule).forEach((slotKey) => {
      ...
      poolData[slotKey].push({
        ...
        professor: student.hasProfessor
          ? (student.professorName || "Profesor Asignado")
          : "Pileta Libre",
      });
    });
  });
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.05) }}>
                <TableCell sx={{ fontWeight: 800, width: 80 }}>Hora</TableCell>
                {DAYS.map(day => (
                  <TableCell key={day} align="center" sx={{ fontWeight: 800 }}>{day}</TableCell>
                ))}
              </TableRow>
            </TableHead>
            <TableBody>
              {HOURS.map(hour => (
                <TableRow key={hour} hover>
                  <TableCell sx={{ fontWeight: 700, color: "text.secondary" }}>{hour}</TableCell>
                  {DAYS.map(day => {
                    const count = poolData[`${day}-${hour}`]?.length || 0;
                    return (
                      <TableCell key={`${day}-${hour}`} align="center" sx={{ p: 1 }}>
                        {count > 0 ? (
                          <Chip
                            label={`${count} alumno${count > 1 ? "s" : ""}`}
                            size="small"
                            color="primary"
                            variant={count > 5 ? "filled" : "outlined"}
                            sx={{ fontWeight: 700 }}
                          />
                        ) : (
