...
export default function CashRegistry({
  ...
  const todayStaff = staffRoster[todayName] || staffRoster["Lunes"] || { morning: "", afternoon: "" };
  ...
  useEffect(() => {
    if (todayStaff.morning || todayStaff.afternoon) {
      setShifts((prev) =>
        prev.map((s, idx) => {
          // Si el responsable está vacío, intentar asignarlo del roster actual
          if (!s.responsible) {
            return {
              ...s,
              responsible: idx === 0 ? todayStaff.morning : todayStaff.afternoon,
            };
          }
          return s;
        }),
